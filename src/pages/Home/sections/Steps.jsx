...
const StepItem = ({ number, title, desc, icon, color, delay }) => (
    <div className={`flex flex-col items-center text-center gap-4 relative group w-full md:w-1/4`}>
        {/* Step Number Sticker */}
        <div className={`absolute -top-4 -left-2 w-10 h-10 rounded-full ${color} text-white font-black flex items-center justify-center text-lg shadow-lg transform -rotate-12 group-hover:rotate-0 transition-transform duration-300 z-20 border-2 border-white`}>
            {number}
        </div>

        {/* Image Container */}
        <div className="relative w-32 h-32 md:w-40 md:h-40 bg-white rounded-full shadow-xl flex items-center justify-center border-4 border-gray-50 group-hover:scale-105 transition-transform duration-300 z-10">
            <img src={icon} alt={title} className="w-20 h-20 md:w-24 md:h-24 object-contain" />
        </div>

        {/* Text */}
        <div className="mt-2">
            <h3 className="text-xl font-bold text-brand-dark mb-1">{title}</h3>
            <p className="text-sm text-gray-500 font-medium leading-tight max-w-[150px] mx-auto">{desc}</p>
        </div>
    </div>
);
		...
		<div className="w-full py-24 px-6 overflow-hidden">
			<div className="container mx-auto max-w-6xl relative">
                ...
                <div className="text-center mb-16 relative z-10">
                    <h2 className="text-4xl md:text-5xl font-black text-brand-dark mb-4">How it works?</h2>
                ...
                {/* Decorative Dashed Line (Desktop) */}
